...
function validarContrato() {
	...
	if (!expresion.test(nombreEmpleador)) {
	...
	else if (!expresion.test(nombreEmpleador)) {
		alert("Los caracteres del nombre del empleador no son válidos");
		return false;
	}
	...
	else if (duracionContrato < -1) {
		alert("Los meses no pueden ser negativos");
		return false;
	}
	else {
		swal({
		  title: "Está seguro que quiere modificar el contrato?",
		...
		.then((willDelete) => {
		  if (willDelete) {
		    ...
		    swal("No se realizaron cambios!");
		    document.getElementById("modificarContrato").reset();
		  }
		});
		return false;
	}
